...
def send_frame(sock, data, opcode=0x02, mask=None):
    ...
    buf = bytes()
    buf += bytes(
        [0b10000000 | (opcode & 0b00001111)]
    )  # FIN bit always set, mask opcode
...
def sample():
    ...
    websocket = list(
        filter(
            lambda x: x.lower().startswith("Sec-WebSocket-Accept".lower()), all_lines
        )
    )
    # print( all_lines )
    ...
    send_frame(
        sock,
        "this is a masked demo text".encode(),
        opcode=0x2,
